-- Phikadfest — ข้อมูลอ้างอิงตั้งต้น: 77 จังหวัด + หมวดหมู่งาน
-- lat/lng เป็นพิกัดตัวเมืองของแต่ละจังหวัดโดยประมาณ
-- ใช้เป็นค่า fallback สำหรับ "งานใกล้ฉัน" เมื่ออีเวนต์ยังไม่มีพิกัดของตัวเอง

insert into categories (slug, name_th, name_en, emoji, sort_order, accent_color) values
  ('annual-fair', 'งานประจำปี / งานวัด',  'Annual Fair',     '🎡', 10,  '#f59e0b'),
  ('music',       'ดนตรี / คอนเสิร์ต',    'Music',           '🎵', 20,  '#a855f7'),
  ('workshop',    'เวิร์กช็อป / เรียนรู้', 'Workshop',        '🛠️', 30,  '#0ea5e9'),
  ('car-meet',    'รถซิ่ง / คาร์มีท',      'Car Meet',        '🏁', 40,  '#ef4444'),
  ('food',        'อาหาร / ตลาดนัด',      'Food & Market',   '🍜', 50,  '#f97316'),
  ('culture',     'ประเพณี / วัฒนธรรม',   'Culture',         '🪷', 60,  '#e11d48'),
  ('sport',       'กีฬา / วิ่ง',           'Sports',          '🏃', 70,  '#22c55e'),
  ('art',         'ศิลปะ / นิทรรศการ',    'Art & Exhibition','🎨', 80,  '#6366f1'),
  ('other',       'อื่นๆ',                'Other',           '✨', 999, '#78716c')
on conflict (slug) do nothing;

insert into tags (slug, name_th) values
  ('free',        'เข้าฟรี'),
  ('family',      'ไปกับครอบครัวได้'),
  ('night',       'งานกลางคืน'),
  ('outdoor',     'กลางแจ้ง'),
  ('pet-friendly','พาสัตว์เลี้ยงได้')
on conflict (slug) do nothing;

insert into provinces (code, slug, name_th, name_en, region, lat, lng) values
  -- ภาคเหนือ
  ...
  ('TH-58', 'mae-hong-son',      'แม่ฮ่องสอน',       'Mae Hong Son',      'north',     19.3020,  97.9654),
  ('TH-55', 'nan',               'น่าน',             'Nan',               'north',     18.7756, 100.7730),
  ('TH-56', 'phayao',            'พะเยา',            'Phayao',            'north',     19.1664,  99.9003),
  ('TH-54', 'phrae',             'แพร่',             'Phrae',             'north',     18.1445, 100.1405),
  ('TH-53', 'uttaradit',         'อุตรดิตถ์',        'Uttaradit',         'north',     17.6200, 100.0993),
  ...
  ('TH-31', 'buriram',           'บุรีรัมย์',        'Buri Ram',          'northeast', 14.9930, 103.1029),
  ('TH-36', 'chaiyaphum',        'ชัยภูมิ',          'Chaiyaphum',        'northeast', 15.8068, 102.0317),
  ('TH-46', 'kalasin',           'กาฬสินธุ์',        'Kalasin',           'northeast', 16.4315, 103.5059),
  ('TH-40', 'khon-kaen',         'ขอนแก่น',          'Khon Kaen',         'northeast', 16.4419, 102.8360),
  ('TH-42', 'loei',              'เลย',              'Loei',              'northeast', 17.4860, 101.7223),
  ('TH-44', 'maha-sarakham',     'มหาสารคาม',        'Maha Sarakham',     'northeast', 16.1850, 103.3007),
  ('TH-49', 'mukdahan',          'มุกดาหาร',         'Mukdahan',          'northeast', 16.5420, 104.7207),
  ...
  ('TH-39', 'nong-bua-lamphu',   'หนองบัวลำภู',      'Nong Bua Lam Phu',  'northeast', 17.2218, 102.4260),
  ('TH-43', 'nong-khai',         'หนองคาย',          'Nong Khai',         'northeast', 17.8783, 102.7420),
  ('TH-45', 'roi-et',            'ร้อยเอ็ด',         'Roi Et',            'northeast', 16.0538, 103.6520),
  ('TH-47', 'sakon-nakhon',      'สกลนคร',           'Sakon Nakhon',      'northeast', 17.1664, 104.1486),
  ('TH-33', 'sisaket',           'ศรีสะเกษ',         'Si Sa Ket',         'northeast', 15.1186, 104.3220),
  ...
  ('TH-41', 'udon-thani',        'อุดรธานี',         'Udon Thani',        'northeast', 17.4138, 102.7870),
  ('TH-35', 'yasothon',          'ยโสธร',            'Yasothon',          'northeast', 15.7921, 104.1452),

  -- ภาคกลาง
  ('TH-15', 'ang-thong',         'อ่างทอง',          'Ang Thong',         'central',   14.5896, 100.4550),
  ...
  ('TH-26', 'nakhon-nayok',      'นครนายก',          'Nakhon Nayok',      'central',   14.2069, 101.2130),
  ...
  ('TH-13', 'pathum-thani',      'ปทุมธานี',         'Pathum Thani',      'central',   14.0208, 100.5250),
  ('TH-67', 'phetchabun',        'เพชรบูรณ์',        'Phetchabun',        'central',   16.4190, 101.1591),
  ...
  ('TH-25', 'prachinburi',       'ปราจีนบุรี',       'Prachin Buri',      'east',      14.0500, 101.3700),
  ('TH-21', 'rayong',            'ระยอง',            'Rayong',            'east',      12.6814, 101.2816),
  ('TH-27', 'sa-kaeo',           'สระแก้ว',          'Sa Kaeo',           'east',      13.8240, 102.0645),
  ...
  -- ภาคใต้
  ('TH-86', 'chumphon',          'ชุมพร',            'Chumphon',          'south',     10.4930,  99.1800),
  ...
  ('TH-94', 'pattani',           'ปัตตานี',          'Pattani',           'south',      6.8692, 101.2550),
  ...
  ('TH-95', 'yala',              'ยะลา',             'Yala',              'south',      6.5410, 101.2810)
